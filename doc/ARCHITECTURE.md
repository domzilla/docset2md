...
### DocC Parser Features

The `DocCParser` handles various DocC JSON content types:

**Content Sections:**
- Preserves original section order from DocC JSON (Return Value, Discussion, etc.)
- Content sections output after Parameters in generated markdown
- Multi-line parameter descriptions properly indented

**Block Content Types:**
- `paragraph`: Text with inline formatting
- `codeListing`: Fenced code blocks with syntax highlighting
- `aside`: Blockquotes with style (Note, Important, Warning, etc.)
- `unorderedList`/`orderedList`: Bullet and numbered lists
- `termList`: Definition lists rendered as bullet lists with bold terms
- `table`: Markdown tables with header rows

**Link Handling:**
- Internal links resolved to relative markdown paths
- External links (https://) rendered as clickable hyperlinks
- Cross-language links resolved based on availability

### Framework Index Files

For Apple DocC docsets, framework `_index.md` files are generated by merging:

1. **Framework Entry Content** (if exists): Abstract, overview, topics, see also
2. **Generated Contents Section**: Alphabetized list of Classes/Structs/Protocols/Enums

This preserves the original framework documentation while adding a navigable index.
...
The search index supports FTS5 query syntax. Query terms are automatically escaped to prevent conflicts with FTS5 keywords (AND, OR, NOT, etc.):

| Query | Description |
|-------|-------------|
| `UIWindow` | Exact term match |
| `view*` | Prefix match (wildcard preserved) |
| `"make key"` | Phrase match |
| `NSURL bookmark*` | Combined: term + prefix match |
| `Support In-App Purchases` | Terms with "and"/"or" work correctly (auto-escaped) |

**Query Escaping:**
- Each term is quoted: `NSURL` → `"NSURL"`
- Wildcards preserved outside quotes: `bookmark*` → `"bookmark"*`
- User-quoted phrases kept intact: `"exact phrase"` → `"exact phrase"`
- FTS5 keywords escaped: `view and controller` → `"view" "and" "controller"`
