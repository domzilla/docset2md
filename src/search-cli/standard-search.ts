 ...
 * This variant does not include language filtering (used by DocC variant).
...
DESCRIPTION
  Full-text search across documentation converted from Dash docsets.
...
OPTIONS
  -t, --type <type>     Filter by entry type (e.g., Function, Class, Method,
                        Constant, Property, Type, Macro, Guide)
  -f, --framework <fw>  Filter by framework or category name
...
QUERY SYNTAX
  ...
  Simple terms:
    array_map           Match entries containing "array_map"
    json encode         Match entries containing both "json" AND "encode"

  Prefix matching:
    array*              Match entries starting with "array" (array_map, array_filter, ...)
    str*                Match entries starting with "str" (strlen, strpos, ...)

  Phrase matching:
    "date time"         Match exact phrase "date time"
    "file get"          Match exact phrase "file get"

  Boolean operators:
    array AND sort      Both terms must be present
    json OR xml         Either term can be present
    string NOT replace  Match "string" but exclude "replace"
    (array OR list) AND sort
                        Combine operators with parentheses

  Column-specific search:
    name:DateTime       Search only in symbol names
    abstract:returns    Search only in descriptions
    type:Function       Search only in entry types
...
EXAMPLES
  ...
  Output formats:
    ./search DateTime --format json
    ./search array --format table --limit 100
