...
import { SearchIndexWriter } from '../../../src/search/SearchIndexWriter.js';
...
describe('SearchIndexWriter', () => {
    ...
    describe('addEntry', () => {
        ...
        it('should handle deprecated and beta flags', () => {
            ...
            const db = new Database(testDbPath, { readonly: true });
            const oldRow = db.prepare('SELECT deprecated, beta FROM entries WHERE name = ?').get('OldClass') as {
                deprecated: number;
                beta: number;
            };
            const newRow = db.prepare('SELECT deprecated, beta FROM entries WHERE name = ?').get('NewClass') as {
                deprecated: number;
                beta: number;
            };
            db.close();
    ...
    describe('FTS5 indexing', () => {
        ...
        it('should support prefix search', () => {
            ...
            writer.addEntry({ name: 'UIViewController', type: 'Class', path: 'uiviewcontroller.md' });
